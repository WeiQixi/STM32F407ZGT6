...
void LCD_Open_Window(uint16_t X0,uint16_t Y0, uint16_t width, uint16_t height){
  ...
  if(dir_flag==0&&lcd_id==0x1963){
    X0=lcd_width-width-X0;
    height=Y0+height-1;
    LCD_CMD=set_xcmd;
    LCD_DAT=X0>>8;
    LCD_DAT=X0&0xFF;
    LCD_DAT=(X0+width-1)>>8;
    LCD_DAT=(X0+width-1)&0xFF;
    LCD_CMD=set_ycmd;
    LCD_DAT=Y0>>8;
    LCD_DAT=Y0&0xFF;
    LCD_DAT=height>>8;
	...
	if(lcd_id==0X9341)
		...
		{  			 		
			...
			}else
			{
				...
				LCD_DAT=Xaddr>>8;
				LCD_DAT=Xaddr&0XFF; 		
				LCD_DAT=(lcd_width-1)>>8;
				LCD_DAT=(lcd_width-1)&0XFF;		 	 			
			}	
			LCD_CMD=set_ycmd; 
			LCD_DAT=Yaddr>>8;
			LCD_DAT=Yaddr&0XFF; 		
			LCD_DAT=(lcd_height-1)>>8;
			LCD_DAT=(lcd_height-1)&0XFF; 			 		
...
void LCD_Init(void){
  ...
  if(lcd_id!=0x9341){
    ...
    if(lcd_id==0x5761) lcd_id=0x1963;
  }
  if(lcd_id==0x9341){
  ...
  if(lcd_id==0x9341){
    ...
    LCD_DAT = (800 - 1) >> 8; // LCD水平像素设置
    LCD_DAT = 800 - 1;
    LCD_DAT = (480 - 1) >> 8; // LCD垂直像素设置
    LCD_DAT = 480 - 1;
    LCD_DAT = 0x00; // RGB序列

    LCD_CMD = 0xB4;
    LCD_DAT = (800 + 46 + 210 - 1) >> 8;
    LCD_DAT = 800 + 46 + 210 - 1;
    LCD_DAT = 46 >> 8;
    LCD_DAT = 46;
    LCD_DAT = 0;
    LCD_DAT = 0x00;
    ...
    LCD_CMD = 0xF0;
    LCD_DAT = 0x03; // SSD1963与MCU的接口为16bit
