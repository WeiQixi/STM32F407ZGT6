...
/* 中断嵌套行为配置 */
/* 
   注意：为避免汇编器报错 "Bad operand types"，这里直接使用计算后的常数值，
   而不是让汇编器去计算移位表达式。
   STM32F4 优先级位数为 4。
   configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY = 5
   计算：5 << (8-4) = 5 << 4 = 80 (0x50)
*/

#define configKERNEL_INTERRUPT_PRIORITY                 255                 /* 最低优先级 (0xFF)，确保 PendSV 和 SysTick 优先级最低 */
#define configMAX_SYSCALL_INTERRUPT_PRIORITY            80                  /* 5 << 4 = 80 (0x50)。所有调用 FreeRTOS API 的中断优先级必须 >= 80 (数值上) */
#define configMAX_API_CALL_INTERRUPT_PRIORITY           configMAX_SYSCALL_INTERRUPT_PRIORITY
