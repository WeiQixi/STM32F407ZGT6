...
// LV_INDEV_DEF_READ_PERIOD 被移除了，控制权交给了你！
/* 例如：
-----------------------------------------------------------
...
if(read_timer){
    将默认的采样周期 (LV_DEF_REFR_PERIOD) 修改为 10ms
    注意：数值越小采样越快，但 CPU 负载越高；数值越大越省电但手感可能迟钝
    lv_timer_set_period(read_timer,10);
    LV_LOG_INFO("Touchpad sampling rate set to 10ms");
}else {
    LV_LOG_WARN("Failed to get read timer for touchpad");
}
此函数放置在 lv_port_indev.c 的 lv_port_indev_init() 函数后
*/
