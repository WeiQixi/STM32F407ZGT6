...
/****************************************************************************
* 名    称: bool XPT2046_GetPressedAndXY(int32_t *x,int32_t *y)
* 功    能：触摸按键扫描	
* 入口参数：坐标 
* 返回参数: 是否被按下
* 说    明：LVGL用     
****************************************************************************/
bool XPT2046_GetPressedAndXY(int32_t *x,int32_t *y){
  if(PEN==1) return false;    // 没按
  uint16_t raw_x,raw_y;
  if(!RTouch_Read_XY2(&raw_x,&raw_y)) return false;
  // 校准
  int32_t sx=(int32_t)(xFactor*raw_x+xOffset);
  int32_t sy=(int32_t)(yFactor*raw_y+yOffset);
  // 统一限幅（根据当前方向）
    if (dir_flag) {
        if (*x < 0) *x = 0;
        if (*y < 0) *y = 0;
